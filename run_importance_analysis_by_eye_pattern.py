...
修正版：標準グリッド（30-2: 76点, 24-2: 54点, 10-2: 68点）を使用
...
GNN_PROJECT_PATH = SCRIPT_DIR  # ★修正: スクリプトのあるディレクトリ
...
SCRIPTS = [
    {
        'name': 'Graph Construction (FIXED)',
        'script': 'create_graph_by_eye_pattern_fixed.py',  # ★修正版
        'description': 'Creating graph data with NORMALIZED coordinates...',
        'critical': True,
        'details': [
            '  ★ FIXED: Coordinates normalized to [-1, 1]',
            '  ★ FIXED: Sensitivity validated [0, 40] dB',
            '  - Eye separation: Left (0) / Right (1)',
            '  - Patterns: 30-2, 24-2, 10-2',
            '  - Features: [NormX, NormY, Distance]'
        ]
    },
    {
        'name': 'Model Training (FIXED)',
        'script': 'train_by_eye_pattern_fixed.py',  # ★修正版
        'description': 'Training GNN models with PROPER loss function...',
        'critical': True,
        'details': [
            '  ★ FIXED: MSE + MAE loss (not NLL)',
            '  ★ FIXED: Output clamped to [0, 40] dB',
            '  - Separate models for each eye-pattern',
            '  - Expected MAE: 3-8 dB'
        ]
    },
    {
        'name': 'Importance Computation (Standard Grid)',
        'script': 'compute_importance_by_eye_pattern_fixed.py',  # ★修正版
        'description': 'Computing importance maps with STANDARD GRID...',
        'critical': False,
        'details': [
            '  ★ Using standard Humphrey grid points:',
            '    - 30-2: 76 points (74 excluding Mariotte)',
            '    - 24-2: 54 points (52 excluding Mariotte)',
            '    - 10-2: 68 points',
    ...
    {
        'name': 'Visualization (Standard Grid)',
        'script': 'visualize_importance_by_eye_pattern_fixed.py',  # ★修正版
        'description': 'Creating visualizations with STANDARD GRID...',
        'critical': False,
        'details': [
            '  ★ Correct point counts:',
            '    - 30-2: 74 points (Mariotte excluded)',
            '    - 24-2: 52 points (Mariotte excluded)',
            '    - 10-2: 68 points',
...
def print_project_summary():
    ...
    print("┃                                                                    ┃")
    print("┃ ★ Standard Grid Points (FIXED):                                   ┃")
    print("┃  • 30-2: 76 points (74 excluding Mariotte)                        ┃")
    print("┃  • 24-2: 54 points (52 excluding Mariotte)                        ┃")
...
def check_prerequisites():
    ...
    # models_revised.pyの確認（オプション - 修正版では不要）
    models_revised_path = SCRIPT_DIR / 'models_revised.py'
    if models_revised_path.exists():
        print(f"   ✓ models_revised.py (optional)")
    else:
        print(f"   ℹ models_revised.py not found (not required for fixed version)")
    
    # standard_grid_definitions.pyの確認（オプション）
    standard_grid_path = SCRIPT_DIR / 'standard_grid_definitions.py'
    if standard_grid_path.exists():
        print(f"   ✓ standard_grid_definitions.py (optional utility)")
    else:
        print(f"   ℹ standard_grid_definitions.py not found (functions embedded in scripts)")
    ...
    output_dirs = [
        ...
        GNN_PROJECT_PATH / "data" / "by_eye_pattern_correct",
        GNN_PROJECT_PATH / "models" / "by_eye_pattern",
        GNN_PROJECT_PATH / "models" / "by_eye_pattern_final",
        GNN_PROJECT_PATH / "results" / "by_eye_pattern",
        GNN_PROJECT_PATH / "results" / "importance_maps_standard",
...
def main():
    ...
    print("  3. Importance score computation ★ STANDARD GRID")
    print("  4. Visualization ★ STANDARD GRID")
    ...
    if all_success:
        ...
        print("="*70)
        print("\nResults location:")
        print(f"  📊 Graph data:      {GNN_PROJECT_PATH / 'data' / 'by_eye_pattern_correct'}")
        print(f"  🤖 Models:          {GNN_PROJECT_PATH / 'models' / 'by_eye_pattern_final'}")
        print(f"  📈 Importance maps: {GNN_PROJECT_PATH / 'results' / 'importance_maps_standard'}")
        print(f"  📉 Visualizations:  {GNN_PROJECT_PATH / 'visualizations'}")
        
        print("\n" + "┏" + "━"*68 + "┓")
        print("┃" + " STANDARD GRID VERIFICATION ".center(68) + "┃")
        print("┣" + "━"*68 + "┫")
        print("┃ Check the visualizations for correct point counts:               ┃")
        print("┃  • 30-2: 74 points (Mariotte excluded)                           ┃")
        print("┃  • 24-2: 52 points (Mariotte excluded)                           ┃")
        print("┃  • 10-2: 68 points                                               ┃")
        print("┗" + "━"*68 + "┛")
        ...
        print("┃    → Verify correct point counts in each pattern                  ┃")
        ...
        print("  5. Ensure _fixed.py scripts are in the same directory")
